...
def create_compositional_language(dataset, num_attributes, num_values, message_length, vocab_size):
    assert num_attributes <= message_length
    assert num_values <= vocab_size

    meanings = []
    messages = []
    for meaning in dataset:
        message = meaning.view(num_attributes, num_values).argmax(dim=-1)
...
if __name__ == '__main__':
    ...
    meanings, messages = create_compositional_language(dataset, num_attributes=config["model"]["num_attributes"], num_values=config["model"]["num_values"], message_length=config["model"]["max_len"], vocab_size=config["model"]["vocab_size"])
