...
for entropy_coeff in 0.3 0.4 0.5
do
	#Baseline
	output_folder=out/train_baseline_${job_id}.out
	baseline_args="--sender-entropy-coeff=$entropy_coeff --receiver-entropy-coeff=$entropy_coeff --log-topsim-on-validation --log-posdis-on-validation --log-bosdis-on-validation --num-features 2 --num-values 16 --vocab-size 50 --max-len 3"
